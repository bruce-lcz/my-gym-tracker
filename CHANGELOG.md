...
- **完整動作管理系統**:
  - 新增 `ExerciseManager` 組件：完整的 CRUD 操作介面
  - 支援新增、編輯、刪除動作，即時同步至 Google Sheets
  - 整合 AI 翻譯功能：輸入英文動作名稱自動生成中文與肌群
  - 搜尋過濾：支援中英文即時搜尋
  - 分類顯示：按身體部位自動分組（胸部、背部、肩部、下肢、手臂/核心、有氧）
  - 視覺化設計：現代卡片式佈局，清晰的操作按鈕

- **上次訓練參考面板**:
  - 新增 `LastWorkoutReference` 組件：顯示上次訓練詳細資訊
  - 智能重量填入：選擇動作後自動填入上次訓練的參考重量
  - 訓練統計：顯示上次重量、最大重量、總組數、總次數
  - 訓練詳情：列出每一組的重量與次數，標示參考組
  - 快速增重按鈕：+2.5kg、+5kg、+10kg，支援漸進式超負荷
  - 一鍵套用：直接使用上次重量或增重後的重量
  - 參考重量邏輯：使用倒數第二組重量（更接近工作組重量）

- **Google Apps Script API 增強**:
  - 新增 `updateExercise()` 端點：支援更新動作資訊
  - 新增 `deleteExercise()` 端點：支援刪除動作
  - 修正 `getExercises()` 欄位映射：正確讀取 Part 欄位
  - 更新 Exercises 表格結構：新增 Part 欄位作為第一欄
  - 改進錯誤處理與驗證機制

- **資料載入與快取優化**:
  - 重構 `exerciseData.ts`：簡化資料載入邏輯
  - 移除欄位偏移檢測（已在後端修正）
  - 改進快取策略：優先使用 Google Sheets，失敗時使用快取
  - 新增 `clearExerciseCache()` 函數：清除本地快取
  - 優化預設動作清單：新增常見動作的完整資訊

- **API 功能擴充**:
  - 新增 `updateExercise()` API 函數
  - 新增 `deleteExercise()` API 函數
  - 修正 `fetchLogs()` 組數順序：使用 `unshift` 確保最新組數在前
  - 更新型別定義：支援 `part` 欄位

- **使用者體驗改進**:
  - 從菜單開始訓練時，智能填入重量（優先菜單重量，其次上次重量）
  - 從動作選單選擇時，自動填入上次訓練重量
  - 訓練參考面板僅在重量訓練時顯示（有氧運動不顯示）
  - 改進表單互動：更流暢的重量填入體驗

- **文件更新**:
  - 新增 `LAST_WORKOUT_REFERENCE.md`：上次訓練參考功能說明
  - 新增 `GOOGLE_APPS_SCRIPT_FULL.js`：完整的 Apps Script 程式碼備份
  - 更新 `AppScript_v2.gs`：包含最新的 API 端點
